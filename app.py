...
def generate_groq_signal(asset: str, price_data: dict, news_data: list, timeframe: str, api_key: str) -> dict:
    ...
    prompt = """
You are an expert institutional trading analyst. Analyze the market structure and news for {asset_name}:
- Timeframe: {tf}
- Current Market Price: {price}
- Daily Change (%): {change}
- Today's High-Impact News: {news}
...
def generate_groq_signal(asset: str, price_data: dict, news_data: list, timeframe: str, api_key: str) -> dict:
    ...
    # Using standard multiline string without f-string prefix to avoid JSON brace conflicts
    prompt = """
You are an expert institutional trading analyst. Analyze the market structure and news for %s:
- Timeframe: %s
- Current Market Price: %s
- Daily Change (%%): %s
- Today's High-Impact News: %s
...
""" % (asset, timeframe, current_price, daily_change, news_str)
...
def generate_groq_signal(asset: str, price_data: dict, news_data: list, timeframe: str, api_key: str) -> dict:
    ...
    prompt = f"""
    You are an expert institutional trading analyst. Analyze the market structure and news for {asset}:
    - Timeframe: {timeframe}
    - Current Market Price: {price_data.get('price')}
    - Daily Change (%): {price_data.get('change')}
    - Today's High-Impact News: {json.dumps(news_data, indent=2)}
    ...
    {{
        ...
        "order_type": "MARKET" | "BUY LIMIT" | "SELL LIMIT" | "BUY STOP" | "SELL STOP",
        "entry_price": 0.0,
        "stop_loss": 0.0,
        "take_profit_1": 0.0,
        "take_profit_2": 0.0,
...
def generate_groq_signal(asset: str, price_data: dict, news_data: list, api_key: str) -> dict:
    """Generate trade signal and reasoning using Groq API with active models."""
    if not api_key:
        return {"signal": "ERROR", "confidence": 0, "reasons": ["Groq API key missing"]}

    client = Groq(api_key=api_key)

    # Note double braces {{ }} around JSON structure to escape them in f-strings
    prompt = f"""
    ...
    st.error("All model fallbacks failed. Check model endpoints or API key permissions.")
    return {"signal": "ERROR", "confidence": 0, "reasons": ["All model fallbacks failed"]}
